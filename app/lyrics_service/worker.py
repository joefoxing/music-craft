...
def process_lyrics_extraction(
    ...
    try:
        ...
        processed = postprocess.postprocess_lyrics(
            ...
            include_word_timestamps=True,  # Always get words for formatting
            deduplicate=True
        )
        ...
        if not result["meta"]["duration_sec"] and transcription_result.get("duration"):
...
if __name__ == "__main__":
    ...
    with Connection(redis_conn):
        worker = Worker([queue], name=f"{config.SERVICE_NAME}-worker")
