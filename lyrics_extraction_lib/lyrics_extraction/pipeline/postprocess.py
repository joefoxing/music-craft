...
def format_lyrics_with_timestamps(
    ...
    max_chars_per_line: int = 60,
    line_gap_threshold: float = 0.3,
    stanza_gap_threshold: float = 1.0,
    enable_uppercase_breaks: bool = True,
    min_chars_before_upper_break: int = 18,
    min_words_before_upper_break: int = 4
    ...
    Args:
        ...
        line_gap_threshold: Time gap (seconds) to trigger new line
        stanza_gap_threshold: Time gap (seconds) to trigger stanza break
        enable_uppercase_breaks: Enable line breaks before uppercase words
        min_chars_before_upper_break: Minimum chars in line before uppercase break
        min_words_before_upper_break: Minimum words in line before uppercase break
    ...
    if has_words:
        formatted, words = format_lyrics_from_words(
            ...
            stanza_gap_threshold=stanza_gap_threshold,
            uppercase_break=enable_uppercase_breaks,
            min_chars_before_upper_break=min_chars_before_upper_break,
            min_words_before_upper_break=min_words_before_upper_break
        )
    ...
    for segment in segments:
        ...
        if gap > stanza_gap_threshold and current_stanza_lines:
        ...
        elif gap > line_gap_threshold or not current_stanza_lines:
            # Medium gap or first line: new line
            if current_stanza_lines and gap > line_gap_threshold:
...
def postprocess_lyrics(
    ...
    Args:
        ...
        line_gap_threshold: Time gap threshold for line breaks
        stanza_gap_threshold: Time gap threshold for stanza breaks
        enable_uppercase_breaks: Enable line breaks at uppercase words
    ...
    if segments:
        logger.debug(f"Processing {len(segments)} segments with timestamps")
        lyrics, word_timestamps = format_lyrics_with_timestamps(
            ...
            line_gap_threshold=line_gap_threshold,
            stanza_gap_threshold=stanza_gap_threshold,
            enable_uppercase_breaks=enable_uppercase_breaks
        )
    elif raw_text:
        # Fallback: clean raw text
        logger.warning("Fallback to raw_text - no line breaks will be added!")
    ...
    logger.info(f"Post-processing complete: language={language_detected}, length={len(lyrics)}")
